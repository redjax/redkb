---
title: "Devops"
date: 2026-07-04T03:06:41-04:00
draft: true
weight: 10
toc: true
keywords: []
tags: []
---

Developer Operations (DevOps) is the integration and automation of software development and IT operations. It is primarily aimed at shortening the development process and improving the development lifecycle. The field has continued to evolve over the years, with sub-specializations such as Platform Engineering (maintaining the "platform" developers build on top of), Site Reliability Engineering (building the infrastructure for monitoring and alerting), Finance Operations (automating financial aspects of software development), Development Security Operations (integrating security steps into the development process), Release Engineering (building & maintaining the processes & automations around releasing software versions) & more.

As software development became more complex and moved more towards the cloud, a niche formed between the actual software development, and everything that comes after, including builds & releases, security, and cloud deployment. The descriptions of each title above are not universal; companies often post positions using the DevOps branch titles interchangeably, and there are many responsibilities shared between the different titles. The specializations above do not necessarily mean that is *all* an engineer will handle, but each title is focused on a different aspect of the overarching "DevOps" umbrella.

## Sections

| Section             | Description                                            |
| ------------------- | ------------------------------------------------------ |
| [Github](./github/) | Documentation for Github Actions & reusable workflows. |
