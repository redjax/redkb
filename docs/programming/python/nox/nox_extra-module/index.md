...
## Check if nox_extra package is detected
if importlib.util.find_spec("nox_extra"):
    ## Import extra nox modules from a "nox_extra/" directory.
    import nox_extra.nox_django_sessions as nox_django_sessions
    ...
    DJANGO_PROJECT_DIR = nox_django_sessions.DJANGO_PROJECT_DIR

    nox_utils.append_lint_paths(
        extra_paths=["weatherdaily", "nox_extra"], lint_paths=DEFAULT_LINT_PATHS
    )

else:
    ## No nox_extra module detected. Start fresh
    logging.basicConfig(
        level="DEBUG",
        format="%(levelname)s | [%(asctime)s] | [%(modulename)s.%(funcName)s:%(lineno)] |> %(message)s",
        datefmt="%Y-%m-%d_%H:%M:%S",
    )

    log.info(
        """No nox helpers found in nox_extra. You must write your own nox sessions in noxfile.py.
        
        In your noxfile.py, you can safely remove the if/else statement and nox_extra imports, leaving everything below the else.
"""
    )
    ...
    DJANGO_PROJECT_DIR: str = "./weatherdaily"

    ## Write your nox sessions below
    #  Reference: https://nox.thea.codes/en/stable/tutorial.html
