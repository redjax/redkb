...
Class controller for interacting with [minio S3 storage](https://min.io/docs/minio/linux/developers/python/minio-py.html).

This script is a CLI, run it with `--help` to see usage instructions. The controller can load Minio configuration from CLI args, or from a JSON configuration file. The script accepts `-g`/`--generate-configs` as an arg, which will create dummy versions of a `minio_config.json` and various "job" files that you can edit for your needs.

The script can be dropped into an existing app so you can import pieces of it into other scripts/packages, or it can be used as a standalone CLI. The `# /// script` portion at the top of the file informs [`uv`](https://docs.astral.sh/uv/guides/scripts/#declaring-script-dependencies) and other package managers on which dependencies the script requires. This is called [inline script metadata](https://packaging.python.org/en/latest/specifications/inline-script-metadata/#script-type), and is a built-in feature of the Python language. This was introduced to the language in [PEP 723](https://peps.python.org/pep-0723/).

`uv` will automatically detect the inline metadata when you run this script with `uv run script_name.py`, and will install the packages declared at the top of the file.

```python title="MinioController.py" linenums="1"
# /// script
...
class MinioJobConfigGenerator:
    ...
    def generate(self):
        ...
        if not os.path.exists(self.output_path) or not os.path.isfile(self.output_path):
            with open(self.output_path, "w") as f:
                json.dump(self.default_config, f, indent=4)
            self.logger.info(f"Generated default {self.job_type} job config at {self.output_path}")
        else:
            self.logger.warning(f"{self.job_type} job config already exists at {self.output_path}")
...
def parse_args() -> argparse.Namespace:
        ...
        parser.add_argument("-g", "--generate-configs", action="store_true", help="Generate default configs")
        parser.add_argument(
            "-c",
            "--minio-config",
            type=str,
...
def load_minio_job(job_file: str) -> t.Union[MinioCopyJob, MinioMoveJob, MinioDeleteJob, MinioUploadJob, MinioDownloadJob]:
...
def get_minio_controller(endpoint: str, access_key: str, secret_key: str, secure: bool = True, cert_check: bool = True) -> "MinioController":
    ...
    try:
        ...
        controller: MinioController = MinioController(endpoint=endpoint, access_key=access_key, secret_key=secret_key, secure=secure, cert_check=cert_check)
...
class MinioController:
    ...
    def __init__(self, endpoint: str, access_key: str, secret_key: str, secure: bool, cert_check: bool) -> None:
    ...
    def _upload(self, bucket: str, local_path: str, remote_path: str) -> None:
        ...
        if Path(local_path).is_file():
            ...
            self.logger.debug(f"Uploading file '{local_path}' to '{remote_path}' in bucket '{bucket}'")
            try:
                self.client.fput_object(bucket, remote_path, local_path)
                self.logger.debug(f"Uploaded '{local_path}' to '{remote_path}' in bucket '{bucket}'")
            except Exception as exc:
                self.logger.error(f"Error uploading '{local_path}' to '{remote_path}' in bucket '{bucket}': {exc}")
                raise
            ...
            for local_file in glob.glob(local_path + "/**", recursive=True):
                ...
                if Path(local_file).is_file():
                    ...
                    self.logger.debug(f"Uploading file '{local_file}' to '{remote_file_path}' in bucket '{bucket}'")
                    try:
                        self.client.fput_object(bucket, remote_file_path, local_file)
                        self.logger.debug(f"Uploaded '{local_file}' to '{remote_file_path}' in bucket '{bucket}'")
                    except Exception as exc:
                        self.logger.error(f"Error uploading '{local_file}' to '{remote_file_path}' in bucket '{bucket}': {exc}")
                        raise
                    
        else:
            ## Path is not a file or directory
            raise FileNotFoundError(f"Local path '{local_path}' does not exist or is not a file/directory.")
    ...
    def _download(self, bucket: str, remote_path: str, local_path: str):
        ...
        if remote_path.endswith("/"):
            ...
            try:
                ...
                if e.code == 'NoSuchKey':
                    raise FileNotFoundError(f"Remote object '{remote_path}' not found in bucket '{bucket}'")
                raise
    ...
    def _copy(self, bucket: str, src_path: str, dest_path: str, dest_bucket: str | None = None):
        ...
        try:
            ...
            try:
                ...
                log.debug(f"Copying object '{src_path}' from bucket '{bucket}' to '{dest_path}' in bucket '{dest_bucket}'")
                ...
                if e.code == 'NoSuchKey':
                    ...
                    objects = list(self.client.list_objects(bucket, prefix=src_path, recursive=True))
                    if objects:
                        ...
                        for obj in objects:
                            ...
                            log.debug(f"Copying object '{obj.object_name}' from bucket '{bucket}' to '{new_dest_path}' in bucket '{dest_bucket}'")
                            self.client.copy_object(dest_bucket, new_dest_path, copy_source)
                    else:
                        ## No such key or directory
                        raise FileNotFoundError(f"Source path '{src_path}' not found in bucket '{bucket}'.") from e
                else:
                    ## Re-raise the S3Error if it's not a NoSuchKey error
                    raise

        except Exception as exc:
            self.logger.error(f"Error copying '{src_path}' from bucket '{bucket}' to '{dest_path}' in bucket '{dest_bucket}': {exc}")
            raise
    ...
    def _delete(self, bucket: str, remote_path: str):
        ...
        if objects:
            ...
            self.logger.debug(f"Deleting [{len(delete_objects)}] object(s) under prefix '{prefix}'")
            try:
        ...
        try:
            ...
            self.logger.debug(f"Deleted single object '{remote_path}' from bucket '{bucket}'")
        except S3Error as e:
            if e.code == "NoSuchKey":
                self.logger.warning(f"Object '{remote_path}' not found in bucket '{bucket}'")
            else:
                self.logger.error(f"Error deleting '{remote_path}' from bucket '{bucket}': {e}")
                raise
        except Exception as exc:
            self.logger.error(f"Error deleting '{remote_path}' from bucket '{bucket}': {exc}")
            raise



    def upload(self, bucket: str, local_paths: t.Union[str, t.List[str]], remote_path: str = ""):
    ...
    def download(self, bucket: str, remote_paths: t.Union[str, t.List[str]], local_path: str):
    ...
    def object_exists(self, bucket: str, remote_path: str) -> bool:
        ...
        try:
            ...
            if e.code == "NoSuchKey":
                ...
                prefix = remote_path + "/"
                objs = list(self.client.list_objects(bucket, prefix=prefix, recursive=True))
                return len(objs) > 0
            elif e.code == "AccessDenied":
                self.logger.error(f"Access denied when checking object: {bucket}/{remote_path}")
                raise
            else:
                raise
    ...
    def search_files(self, bucket: str, prefix: str) -> t.List[str]:
        ...
        """
        self.logger.debug(f"Searching minio bucket '{bucket}' for files with prefix '{prefix}'")

        try:
            results = [obj.object_name for obj in self.client.list_objects(bucket, prefix=prefix, recursive=True)]
            self.logger.debug(f"Found [{len(results)}] file(s) with prefix '{prefix}' in minio bucket '{bucket}'")

            return results
        except Exception as exc:
            self.logger.error(f"Error searching minio bucket '{bucket}' for files with prefix '{prefix}': {exc}")
            raise
        
    def copy(self, bucket: str, src_path: str, dest_path: str, dest_bucket: str | None = None):
        """Copies a file or directory (prefix) from one location to another within MinIO.
    ...
    def move(self, bucket: str, src_path: str, dest_path: str, dest_bucket: str | None = None):
        ...
        try:
                ...
                if e.code == 'NoSuchKey':
                    ...
                    if objects:
                        for obj in objects:
                            ...
                            log.debug(f"Removed object '{obj.object_name}' from bucket '{bucket}'")
                    else:
                        raise FileNotFoundError(f"Source path '{src_path}' not found in bucket '{bucket}'.") from e
                else:
                    ## Re-raise the S3Error if it's not a NoSuchKey error
            ...
            self.logger.error(f"Error moving '{src_path}' from bucket '{bucket}' to '{dest_path}' in bucket '{dest_bucket}': {exc}")
            raise
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        format="%(asctime)s | [%(levelname)s] | %(name)s:%(lineno)s :: %(message)s" if args.debug else "%(asctime)s [%(levelname)s] :: %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )
    ...
    if args.minio_config and Path(args.minio_config).exists():
        ...
        log.warning(f"Minio config file not found at path '{args.minio_config}'. Generating default config")
    ...
    if not all([endpoint, access_key, secret_key]):
        log.error("Missing MinIO configuration.  Please provide endpoint, access_key, and secret_key via command line or minio_config.json.")
        exit(1)

    ## Initialize MinIO Controller
    controller = get_minio_controller(endpoint, access_key, secret_key, secure=secure, cert_check=cert_check)
    ...
    if args.job_file:
        try:
            ...
            if isinstance(job, MinioUploadJob):
                ...
                controller.copy(bucket=job.bucket, src_path=job.remote_path, dest_path=job.remote_dest, dest_bucket=job.dest_bucket)
            elif isinstance(job, MinioMoveJob):
                controller.move(bucket=job.bucket, src_path=job.remote_path, dest_path=job.remote_dest, dest_bucket=job.dest_bucket)
